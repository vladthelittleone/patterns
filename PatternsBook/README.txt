...
Для примеру Menu и MenuItem, являются практически одинаковыми. Методы, которые не поддерживаются другой реализацией, выкидывают исключение. 

13) Паттерн Состояние - управляет изменением поведения объекта при изменении его внутреннего состояния.
Внешне это выглядит так, словно объект меняет свой класс. Например есть несколько состояний, для проверки которых нужно куча
проверок (if-else). Вместо этого мы состояние инкапсулируем в отдельный класс и при вызове объекта, вызываем соответствующий метод
состояния, в котором выполняем соответствующие действия.

14) Паттерн заместитель (Proxy) - представляет суррогатный объект,управляющий доступос к другому объекту.
...
6. Код должен зависеть от абстракции, а не от конкретных классов - принцип инверсии зависимостей.

7. Взаимодействуйте только с "друзьями" - принцип минимальной информированности. (т.е. такие вещи недопустимы - station.getThermometer.getTemperature())

8. Не вызывайте нас - мы вас сами вызовем (Голливудский принцип). (т.е. компоненты высокого уровня (абстрактный класс) сами вызывают компоненты низкого уровня (реализации). Пример реализации принципа - шаблонный метод, где класс высокого уровня через шаблонный метод управляет компонентами низкого уровня. Компоненты низкого уровня обращаются к компоненте высокого уровня не на прямую.)
